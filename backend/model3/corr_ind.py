...
def corr_ind_1(quota):
    ...
    f = open('/data/prepare.json', 'r')
    prepare = json.load(f)
    f.close()

    ind_copp = {prepare['copp']: {}}

    f = open('/data/zcfzb.csv', 'r', encoding='gbk')
    reader = csv.reader(f)
    next(reader)
    next(reader)
    for line in reader:
...
def corr_ind_2(quota):
    ...
    if quota in ('净利润', '营业总支出', '营业总收入'):
        f = open('/data/lrb.csv', 'r', encoding='gbk')
    else:
        f = open('/data/xjlb.csv', 'r', encoding='gbk')
    reader = csv.reader(f)
    next(reader)
    next(reader)
    for line in reader:
    ...
    f = open('/data/ind_dt.json', 'w')
    json.dump(data, f, ensure_ascii=False, indent=2)
    f = open('/data/corr_ind.json', 'w', encoding='utf-8')
    json.dump({prepare['ind']: corr_ind}, f, ensure_ascii=False, indent=2)
    f = open('/data/ind_ratio.json', 'w')
    json.dump(ind_ratio, f, ensure_ascii=False, indent=2)
    f = open('/data/ind_line.json', 'w')
