...
def corr_ind_obj(c_ind, dis):
    print('corr_ind_obj')
    f = open('/data/ind_dt.json', 'r')  # 折线图数据
    ind_dt = json.load(f)
    f = open('/data/ind_ratio.json', 'r')  # 折线图数据
    ind_ratio = json.load(f)
    f = open('/data/ind_line.json', 'r')  # 折线图标签
