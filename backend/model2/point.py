...
def point(level):
    ...
    f = open('/data/result_detail.csv', 'r', encoding='gbk')
    reader = csv.reader(f)
    next(reader)
    for line in reader:
    ...
    f = open('/data/eclat.json', 'r')
    eclat = json.load(f)
    f.close()

    f = open('/data/frequent_{}.json'.format(level), 'r')
    fre = json.load(f)
    f.close()

    f = open('/data/frequent_2.json', 'r')
    fre2 = json.load(f)
    f.close()
    ...
    f = open('/data/point_{}.json'.format(level), 'w')
    json.dump(point, f, ensure_ascii=False, indent=2)
    f.close()   # 全量散点分布

    # f = open('/data/circul_{}.json'.format(level), 'w')
