...
def colour(loc_i, level):
    ...
    if level == 1:
        ...
        f = open('/data/frequent_{}.json'.format(level-1), 'r')
        fre = json.load(f)
        f.close()
        for i in fre:
    ...
    f = open('/data/frequent_{}.json'.format(level), 'w')
...
def eclat_cluster(loc_i, level):
    ...
    f = open('/data/eclat_{}.json'.format(level), 'r')
    data_obj = json.load(f)
    f.close()

    prepare['loc_set'][level - 1] = loc_i
    f = open('/data/prepare.json', 'w')
    ...
    f = open('/data/eclat_{}.json'.format(level+1), 'w')
